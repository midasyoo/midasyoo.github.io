...
| | 무엇을 보나 | 가입 | 상태 |
|---|---|---|---|
| **1단계 · 조회 수** | 페이지별로 몇 번 열렸는지 | 불필요 | **이미 켜져 있음** |
| **2단계 · 상세 통계** | 유입 경로·국가·기기·시간대 | 필요 (무료, 2분) | 꺼짐 |

1단계만으로도 "어느 프로젝트가 실제로 열리는지"는 바로 알 수 있다.
...
[hits.sh](https://hits.sh/)를 쓴다. 가입도 계정도 없고, 경로가 곧 집계 키다.
...
- **[방문 통계 페이지](stats.html)** — 전 프로젝트 조회 수를 표로
- **홈페이지 바닥글** — 홈페이지 조회 수

> 통계 페이지를 열면 각 항목이 1씩 올라간다. hits.sh가 숫자만 읽어오는 방법을
> 제공하지 않아, 배지를 불러오는 것이 곧 집계이기 때문이다.
> 그래서 자동으로 불러오지 않고 버튼을 눌렀을 때만 가져온다.
...
- hits.sh는 외부 무료 서비스라 언제든 중단될 수 있다.
